...
findPositionPatterns'' chord tuning from count = 
  map (map ((+from) . fromJust) . map (uncurry (flip (findIndex . superEquiv) ))) $ 
    map (zipWith (,) (frettedGuitarStringsLengths from count tuning)) $ 
      notePatterns chord tuning from count

notePatterns chord tuning from count = 
  mapM (filter (flip (any . superEquiv) (chordToNotes chord))) 
